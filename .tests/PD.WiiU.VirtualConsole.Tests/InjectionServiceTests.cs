...
public class InjectionServiceTests
{
    ...
    [TestMethod]
    public async Task InjectAsync_IconInMeta_ReturnsItsBytesBeforeTheWorkFolderGoes()
    {
        var images = new FakeImageConverter();
        var service = Service(images: images, packer: new FakeTitlePacker());
        var injection = new Injection(TestTitle.Base(), Rom(), TestTitle.Game()) { Artwork = new Artwork { Icon = @"C:\art\icon.png" } };

        var result = await service.InjectAsync(injection, Work(), Output());

        Assert.IsNotNull(result.IconTga);
        Assert.AreEqual(18, result.IconTga!.Length, "the fake converter writes an 18-byte file");
    }

    [TestMethod]
    public async Task InjectAsync_NoIconAnywhere_ReturnsNullIcon()
    {
        var service = Service(packer: new FakeTitlePacker());

        var result = await service.InjectAsync(new Injection(TestTitle.Base(), Rom(), TestTitle.Game()), Work(), Output());

        Assert.IsNull(result.IconTga);
    }
