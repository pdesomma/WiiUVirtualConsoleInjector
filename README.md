...
Turns a ROM or disc image into an installable Wii U title, using a stock Virtual Console game as the base. A rewrite of [UWUVCI AIO](https://github.com/stuff-by-3-random-dudes/UWUVCI-AIO-WPF): same recipes, no bundled tools, no bundled keys, runs on Windows, Linux and macOS.
...
- A title key for each base you use. Also entered on **Bases & Keys**; the base is then downloaded from Nintendo's servers.
...
1. **Inject**: pick the console, the base, the ROM, artwork and options, then inject. The finished title lands in the output folder and, if a card is set, under `install/` on the SD card.
